import { Building2, Calendar, FileText, Landmark, TrendingUp, Users } from "lucide-react"
...
const IR_SECTIONS = [
  { icon: FileText, title: "Công bố thông tin", body: "Thông tin công bố theo nghĩa vụ của công ty đại chúng và tổ chức niêm yết." },
  { icon: TrendingUp, title: "Báo cáo định kỳ", body: "Báo cáo tài chính, báo cáo thường niên và các báo cáo định kỳ do Vietbank công bố." },
  { icon: Users, title: "Đại hội đồng cổ đông", body: "Tài liệu, nghị quyết và thông tin liên quan đến các kỳ Đại hội đồng cổ đông." },
  { icon: Landmark, title: "Điều lệ & quản trị", body: "Điều lệ, quy chế nội bộ và tài liệu quản trị công ty." },
]
    ...
    <>
        ...
        <div className="mt-6 grid gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-3">
          {[["VBB", "Mã chứng khoán"], ["HOSE", "Sàn niêm yết"], ["14/07/2026", "Ngày giao dịch chính thức"]].map(([value, label]) => (
            <div key={label} className="bg-surface p-6">
              <div className="font-display text-3xl font-semibold text-navy tnum">{value}</div>
      ...
      <Section tone="soft">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {H1_2026.map((item) => (
            <article key={item.label} className="rounded-xl border border-line bg-surface p-5">
              <Building2 className="h-5 w-5 text-navy" />
              <div className="mt-4 font-display text-2xl font-semibold text-navy tnum">{item.value}</div>
              <h3 className="mt-2 text-[13.5px] font-semibold text-ink">{item.label}</h3>
              <p className="mt-1 text-[12.5px] leading-relaxed text-ink-soft">{item.note}</p>
            </article>
          ))}
        </div>
        <p className="mt-4 text-[12.5px] leading-relaxed text-ink-soft">Mốc dữ liệu: 30/06/2026 hoặc 6 tháng đầu năm 2026 theo từng chỉ tiêu. Báo cáo/công bố gốc trên Vietbank là nguồn chi tiết.</p>
      </Section>
      ...
      <Section>
        ...
        <div className="mt-6 grid gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-4">
          {[["+17%", "Mục tiêu tăng tổng tài sản"], ["+23,6%", "Mục tiêu tăng tín dụng"], ["+23,9%", "Mục tiêu tăng huy động"], ["2.100 tỷ", "Mục tiêu lợi nhuận trước thuế"]].map(([value, label]) => (
            <div key={label} className="bg-surface p-6">
              <div className="font-display text-2xl font-semibold text-navy tnum">{value}</div>
              <p className="mt-2 text-[13px] leading-relaxed text-ink-soft">{label}</p>
            </div>
          ))}
        </div>
