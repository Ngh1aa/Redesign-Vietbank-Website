...
export function Section({
  ...
  return (
    <section id={id} className={`border-b border-line ${bg}`}>
      <div className="mx-auto max-w-[1240px] px-6 py-11 lg:py-14">{children}</div>
    </section>
  )
}
...
export function SectionHead({
  ...
  return (
    <div className="max-w-[720px]">
      ...
      <h2 className="mt-3 font-display text-[clamp(1.65rem,2.6vw,2.15rem)] font-semibold leading-[1.18] tracking-[-0.02em]">{title}</h2>
      {lead && <p className="mt-3 max-w-2xl text-[15.5px] leading-[1.7] text-ink-soft">{lead}</p>}
    </div>
  )
}
...
export function FeatureRow({
  ...
  return (
    <Link to={to} className="group flex flex-col gap-3 py-5 transition-colors sm:flex-row sm:items-start sm:gap-6">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-md bg-navy-050 text-navy transition-colors group-hover:bg-navy group-hover:text-white">
        <Icon className="h-5 w-5" aria-hidden="true" />
      </span>
      <div className="flex-1">
        <h3 className="text-[17px] font-semibold tracking-[-0.01em] group-hover:text-navy">{title}</h3>
    ...
    <section id="contact" className="scroll-mt-28 border-b border-line bg-navy-700 text-white">
      <div className="mx-auto flex max-w-[1240px] flex-col items-start justify-between gap-6 px-6 py-12 lg:flex-row lg:items-center">
        <div className="max-w-2xl">
          <h2 className="font-display text-[clamp(1.55rem,2.4vw,2rem)] font-semibold leading-tight tracking-[-0.015em]">{title}</h2>
